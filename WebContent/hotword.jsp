<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Insert title here</title>
 <script src="echarts.js"></script>
 <script type="text/javascript" src="jquery.main.js"></script>
 <script src="https://cdn.staticfile.org/jquery/3.2.1/jquery.min.js"></script>
 <script src="worldcloud.js"></script>
 <script src="https://cdn.staticfile.org/popper.js/1.15.0/umd/popper.min.js"></script>
 <script src="https://cdn.staticfile.org/twitter-bootstrap/4.3.1/js/bootstrap.min.js"></script>

</head>
<body>
点击查询热词： <input type="submit" value="查询" onclick="search()">
    <div id="main" style="width: 100%; height: 600px"></div>
        <script>
       
        function search() {
        	//var date=$("#date").val();
          var names=[];
           alert("查找成功");
            $.post(
                'Selvlethotword',
                {},
                function(msg){
                	var json=JSON.parse(msg);
                    var size=json.length;
                    //alert(size);
                   // var nums=[];
                    for(i=0;i<size;i++){
                    	var gd1 = "{\"value\": "+Math.ceil(Math.random()*1000000)+",\"name\": '"+json[i].title+"'}";
                    	//alert(json[1].title);
                    	//var gd1={};
                    	//gd1["name"]=json[i].title;
                    	//gd1["value"]=Math.ceil(Math.random()*1000000);
                    	//alert(gd1);
                       names.push(gd1);
                       // alert(names);
                       // nums.push(json[i].num);
                       i++;
                    }
                    alert(names);
              
            //alert(names);
       //     onload = function () {
            	alert("1");
                var data = {
                    value: names,
                   
                 /*  value: [
                        {"value": 8708360, "name": "张继科换 情侣  头像"},
                        {"value": 8262949, "name": "何猷君 文物 上 涂鸦"},
                        {"value": 7412498, "name": "微信 520元 红包"}, 
                        {"value": 6344322, "name": "谷歌  停止  华为  合作"}, 
                        {"value": 6098739, "name": "男子 验 出月 经 推迟"}, 
                        {"value": 5623715, "name": "江苏  校园 反杀案"}, 
                        {"value": 5151726, "name": "圆通  速递  猥亵  客户"}, 
                        {"value": 4611534, "name": "日本  偶遇  梁朝 伟"}, 
                        {"value": 4538623, "name": "梅西获金靴"}, 
                        {"value": 4048528, "name": "哈文为 女儿  庆生"}, 
                        {"value": 3689370, "name": "姜潮 升级 当爸"}, 
                        {"value": 3450898, "name": "何猷君 回应 上热搜"}, 
                        {"value": 3239605, "name": "男婴  丢失  自导 自演"}, 
                        {"value": 3020793, "name": "杭州  一天 桥撞塌"}, 
                        {"value": 2982713, "name": "合照  杀手  王祖贤"}, 
                        {"value": 2835806, "name": "赵帅  世锦赛  金牌"}, 
                        {"value": 2591560, "name": "杭州  多名  保安 被捅"}, 
                        {"value": 2528138, "name": "成都  发现  唐代  城墙"}, 
                        {"value": 2338099, "name": "张紫妍案 调查结果"}, 
                        {"value": 2293437, "name": "A股 公司 爱囤房"}, 
                        {"value": 1055866, "name": "谷歌  停止  部分  合作"}, 
                        {"value": 7624378, "name": "刘士余 主动  投案"}, 
                        {"value": 4959805, "name": "酒后 砸 高铁  玻璃"}, 
                        {"value": 4689591, "name": "首都机场  航班 备降"}, 
                        {"value": 4316816, "name": "靓号被 电视剧 使用"}, 
                        {"value": 3928153, "name": "施瓦辛格  被人 飞踹"}, 
                        {"value": 3863814, "name": "联想  造谣者  道歉"}, 
                        {"value": 3670612, "name": "埃及  巴士  炸弹  袭击"}, 
                        {"value": 3200600, "name": "高三  女孩 弃考 救母"}, 
                        {"value": 2925526, "name": "郑姝音遭 争议 判罚"}, 
                        {"value": 2243115, "name": "长沙 公交 连撞  10 车"}, 
                        {"value": 2154974, "name": "散步  晕倒  男婴  被盗"}, 
                        {"value": 2133664, "name": "松原  5.1 级 地震"}, 
                        {"value": 2651968, "name": "印第安纳州  枪击案"}, 
                        {"value": 2281029, "name": "苏志 燮赵 恩情  恋情"}
                    ],
                   */
                    //小鸟图片
                    image: "data:image/png;base64,"
                }
                var myChart = echarts.init(document.getElementById('main'));
                //温馨提示：image 选取有严格要求不可过大；，否则firefox不兼容  iconfont上面的图标可以
                var maskImage = new Image();
                maskImage.src = data.image
 
                maskImage.onload = function () {
                	alert(2);
                    myChart.setOption({

                        backgroundColor: '#fff',
                        tooltip: {
                            show: false
                        },
                        series: [{
                                type: 'wordCloud',
                                gridSize: 1,
                                sizeRange: [12, 55],
                                rotationRange: [-45, 0, 45, 90],
                                maskImage: maskImage,
                                textStyle: {
                                    normal: {
                                        color: function () {
                                            'rgb(' +
                                                    Math.round(Math.random() * 255) +
                                                    ', ' + Math.round(Math.random() * 255) +
                                                    ', ' + Math.round(Math.random() * 255) + ')'
                                       }
                                    }
                                },
                                left: 'center',
                                top: 'center',
                                // width: '96%',
                                // height: '100%',
                                right: null,
                                bottom: null,
                                // width: 300,
                                // height: 200,
                                // top: 20,
                                data: data.value
                            }]
                    })
                }
 
          //  }
                }
                //"json"
            );
        }
        </script>
</body>
</html>